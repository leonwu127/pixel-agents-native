...
| S5.3 | `src/systems/hook_events.lua` — map 11 Claude hook event types to world actions (SessionStart→spawn, Stop→turn_end, PreToolUse→tool_start, PermissionRequest→permission bubble, etc.); extend `src/world.lua` with `permission`+`prompt_submit` event kinds | `luajit spec/hook_events_spec.lua` passes: 11 event kinds each map to expected world event; unknown events return nil; malformed events return nil | S5.1 | cc:done |
| S5.4 | Wire `scenes/office.lua` — drain `hook_events` channel, set per-character `hookDelivered=true`, suppress polling timers for that session; extend `config.lua` with `hooksEnabled`; `main.lua` lifecycle: install on boot (opt-in), uninstall on quit; update manual-test checklist | With `hooksEnabled=true`, character reacts to Stop within 100ms (vs 500ms+ polling); with `hooksEnabled=false`, existing polling behavior unchanged; clean shutdown removes hooks from settings.json | S5.2, S5.3 | cc:done |
